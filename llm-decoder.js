/* ============================================================
   ...
   📁 kesem-llm/llm-decoder.js
   🔥 Tumpukan N blok transformer DENGAN causal mask (autoregressive
      — token cuma boleh "lihat" dirinya & sebelumnya), ditambah
      proyeksi akhir ke logits vocab. Inilah jalur utama generasi
      teks KESEMPATAN LLM (arsitektur GPT-style, decoder-only).
...
(function () {
    ...
    if (window.__LLMDecoderLoaded) {
        return;
    }
    window.__LLMDecoderLoaded = true;
    ...
    function requireDeps() {
        if (!window.LLMEmbedding) throw new Error('[LLMDecoder] window.LLMEmbedding belum dimuat');
        if (!window.LLMAttention) throw new Error('[LLMDecoder] window.LLMAttention belum dimuat');
        if (!window.LLMTransformer) throw new Error('[LLMDecoder] window.LLMTransformer belum dimuat');
        return { E: window.LLMEmbedding, A: window.LLMAttention, T: window.LLMTransformer };
    }
