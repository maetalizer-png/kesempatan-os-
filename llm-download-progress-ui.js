/* ============================================================
   KESEMPATAN OS - CORE ENGINE v2 DOWNLOAD PROGRESS UI
   📁 llm-download-progress-ui.js

   Progress bar mengambang dengan persentase real-time selama Core
   Engine v2 (llm-transformers-bridge.js) mengunduh berkas model
   pertama kali. Murni presentasi — tidak menyentuh logika unduh/cache
   sama sekali, cuma berlangganan window.KesempatanLLM2.onProgress().
   Setelah berkas dicache di IndexedDB, inisialisasi berikutnya nyaris
   instan sehingga bar ini nyaris tidak pernah muncul lagi.
   ============================================================ */

(function () {
    'use strict';

    if (window.__LLMDownloadProgressUILoaded) {
        return;
    }
    window.__LLMDownloadProgressUILoaded = true;
    ...
    function subscribeWhenReady() {
        if (window.KesempatanLLM2 && typeof window.KesempatanLLM2.onProgress === 'function') {
            window.KesempatanLLM2.onProgress(handleProgress);
            return;
        }
        setTimeout(subscribeWhenReady, 200);
    }
