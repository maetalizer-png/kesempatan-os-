...
*Terakhir diperbarui: Koreksi cockpit — brand header (logo/tagline ukuran asli) kembali IKUT SCROLL bersama konten, hanya bar hamburger tipis (56px, blur+border-bottom) yang tetap fixed di atas; label panel dikembalikan ke Bahasa Inggris (PARAMETER INPUT sebagai heading di atas kartu, ANALYSIS INSTRUCTIONS/SELECT FILE/AUDIO INPUT/RUN ANALYSIS/Execution Mode/Select Agents); satu input gabungan tinggi ±96px auto-grow menggantikan chip pratinjau; kontrol dipadatkan jadi icon-button ≤44px + tombol teks kecil "Agents (recommended: N)" + CTA ≤48px; scrollbar hijau global disembunyikan khusus di `#mainScroll`. Diverifikasi lewat Playwright (scroll 600px, regresi 25 halaman, 42/42 unit test), KES LOG/Report Dock/Export/threshold tidak diubah.*

*Sebelumnya: Koreksi panel "PARAMETER INPUT" jadi satu kartu tunggal — badge mode (AUTO/SEQUENTIAL/PARALLEL/HITL) jadi tombol yang membuka bottom-sheet, pemilihan agen (Select All/Deselect/rekomendasi otomatis) dipindah penuh ke bottom-sheet terpisah, baris status "Intelligence Loop" dihapus, topbar mobile dipadatkan (~90-96px, dari ~246px) sehingga seluruh kartu PARAMETER INPUT muat dalam satu layar (100dvh) tanpa scroll di 360×640 maupun 412×915 — diverifikasi lewat Playwright assert (`cardBottom ≤ viewportHeight`, tanpa scroll internal kartu), regresi 25 halaman, dan 42/42 unit test.*
