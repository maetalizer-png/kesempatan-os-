...
const getApiKey = function() {
    ...
    if (CONFIG.API_KEYS && CONFIG.API_KEYS.openrouter) return CONFIG.API_KEYS.openrouter;
    return null;
};
...
const addCustomAgent = async function(name, role, systemPrompt, temperature, tags, category) {
    ...
    AGENTS_CONFIG[id] = { name: name, role: role, systemPrompt: systemPrompt, temperature: agent.temperature, maxTokens: 800, fewShotExamples: [] };
    if (!CONFIG.AGENTS.includes(id)) CONFIG.AGENTS.push(id);
...
const editCustomAgent = function(id, name, role, systemPrompt, temperature, tags, category) {
    ...
    if (AGENTS_CONFIG[id]) {
        AGENTS_CONFIG[id].name = name;
        AGENTS_CONFIG[id].role = role;
        AGENTS_CONFIG[id].systemPrompt = systemPrompt;
        AGENTS_CONFIG[id].temperature = agent.temperature;
    }
    state.saveAgents();
...
const deleteCustomAgent = function(id) {
    ...
    delete AGENTS_CONFIG[id];
    const idx = CONFIG.AGENTS.indexOf(id);
    if (idx !== -1) CONFIG.AGENTS.splice(idx, 1);
...
KESEMPATAN.CustomAICore = CustomAICore;