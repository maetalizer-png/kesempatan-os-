/* ============================================================
   ...
   📁 kesem-llm/llm-checkpoint.js
   🔥 Bungkus SATU model utuh (config+vocab+merges+bobot lewat
      llm-weights.js) jadi 1 objek checkpoint versi+metadata, simpan/muat
      dari localStorage (browser, tidak ada akses filesystem), dan
      pulihkan jadi model siap pakai LLMRuntime.generate() lagi.
...
(function () {
    ...
    if (window.__LLMCheckpointLoaded) {
        return;
    }
    window.__LLMCheckpointLoaded = true;
    ...
    function requireQuantization() {
        if (!window.LLMQuantization) {
            throw new Error('[LLMCheckpoint] window.LLMQuantization belum dimuat');
        }
        return window.LLMQuantization;
    }
    ...
    function packQuantizedMatrix(q) {
        return {
            ...
            dataB64: window.LLMWeights.typedArrayToBase64(q.data)
        };
    }
    ...
    function unpackQuantizedMatrix(packed) {
        return {
            ...
            data: window.LLMWeights.base64ToInt8Array(packed.dataB64)
        };
    }
    ...
    function restoreModelFromCheckpoint(checkpoint) {
        const Q = requireQuantization();
        if (!window.LLMVocabulary) {
            throw new Error('[LLMCheckpoint] window.LLMVocabulary belum dimuat');
        }
